...
function Pagination({ totalPages }) {
    ...
    return (
        <div className="flex justify-center items-center w-fit my-10 gap-2 mx-auto border-2 border-blue-800 rounded-full">
            ...
            <span className="text-white">{currentPage} <Link href="" className="text-blue-400" onClick={() => handlePageChange(currentPage + 1)}>{currentPage + 1}</Link> <Link className="text-blue-500" href="" onClick={() => handlePageChange(currentPage + 2)}>{currentPage + 2}</Link> ... <Link className="text-blue-600" href="" onClick={() => handlePageChange(totalPages)}>{totalPages}</Link></span>
